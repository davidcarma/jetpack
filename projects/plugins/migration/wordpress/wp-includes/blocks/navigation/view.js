					...
					core: {
						navigation: {
							...
							menuOpenedBy: ( { context } ) =>
								context.core.navigation[
									context.core.navigation.type === 'overlay' ? 'overlayOpenedBy' : 'submenuOpenedBy'
								],
						},
					},
				},
						...
						navigation: {
							openMenuOnHover( store ) {
								...
								if (
									...
									Object.values( navigation.overlayOpenedBy || {} ).filter( Boolean ).length === 0
								)
								...
								if ( selectors.core.navigation.menuOpenedBy( store ).click ) {
									...
									if ( context.core.navigation.type === 'overlay' && event.key === 'Tab' ) {
										...
										if (
											...
											! event.shiftKey &&
											window.document.activeElement === context.core.navigation.lastFocusableElement
										) {
											event.preventDefault();
							...
							handleMenuFocusout: store => {
								...
								if (
									...
									( ! context.core.navigation.modal?.contains( event.relatedTarget ) &&
