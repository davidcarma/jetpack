...
*/
		...
		/***/ 9894: /***/ function ( module, __unused_webpack_exports, __webpack_require__ ) {
		...
		/***/ 2652: /***/ function ( module, __unused_webpack_exports, __webpack_require__ ) {
			/**
			 * Copyright (c) 2013-present, Facebook, Inc.
			 *
			 * This source code is licensed under the MIT license found in the
		...
		/***/ 773: /***/ function ( __unused_webpack_module, exports, __webpack_require__ ) {
			'use strict';
	...
	! ( function () {
		...
		__webpack_require__.d( __webpack_exports__, {
			...
			EditorKeyboardShortcuts: function () {
				return /* reexport */ EditorKeyboardShortcuts;
			},
			...
			PageAttributesOrder: function () {
				return /* reexport */ PageAttributesOrderWithChecks;
			},
			...
			PostPreviewButton: function () {
				return /* reexport */ PostPreviewButton;
			},
			...
			PostSyncStatus: function () {
				return /* reexport */ PostSyncStatus;
			},
			PostSyncStatusModal: function () {
				return /* reexport */ PostSyncStatusModal;
			},
			...
			VisualEditorGlobalKeyboardShortcuts: function () {
				return /* reexport */ VisualEditorGlobalKeyboardShortcuts;
			},
			Warning: function () {
			...
			useEntitiesSavedStatesIsDirty: function () {
				return /* reexport */ useIsDirty;
			},
		...
		__webpack_require__.d( actions_namespaceObject, {
			...
			__unstableSaveForPreview: function () {
				return __unstableSaveForPreview;
			},
			autosave: function () {
				return autosave;
			},
		...
		function hasSameKeys( a, b ) {
			...
			return (
				keysA.length === keysB.length && keysA.every( ( key, index ) => keysB[ index ] === key )
			);
		}
		...
		var external_wp_blocks_namespaceObject = window[ 'wp' ][ 'blocks' ]; // CONCATENATED MODULE: external ["wp","date"]
		 ...
		 */
		const hasEditorUndo = ( 0, external_wp_data_namespaceObject.createRegistrySelector )(
			select => () => {
		 ...
		 */
		const hasEditorRedo = ( 0, external_wp_data_namespaceObject.createRegistrySelector )(
			select => () => {
		...
		const isEditedPostDirty = ( 0, external_wp_data_namespaceObject.createRegistrySelector )(
			select => state => {
				...
				if (
					select( external_wp_coreData_namespaceObject.store ).hasEditsForEntityRecord(
						'postType',
						postType,
						postId
					)
		...
		function getCurrentPostRevisionsCount( state ) {
			...
			return ( _getCurrentPost$_link =
				getCurrentPost( state )._links?.[ 'version-history' ]?.[ 0 ]?.count ) !== null &&
		...
		function getCurrentPostLastRevisionId( state ) {
			...
			return ( _getCurrentPost$_link2 =
				getCurrentPost( state )._links?.[ 'predecessor-version' ]?.[ 0 ]?.id ) !== null &&
		 ...
		 */
		const getPostEdits = ( 0, external_wp_data_namespaceObject.createRegistrySelector )(
			select => state => {
				const postType = getCurrentPostType( state );
				const postId = getCurrentPostId( state );
				return (
					select( external_wp_coreData_namespaceObject.store ).getEntityRecordEdits(
						'postType',
						postType,
						postId
					) || EMPTY_OBJECT
		 ...
		 * @return {*} Post attribute value.
		 */
		const getNestedEditedPostProperty = rememo(
			( state, attributeName ) => {
				const edits = getPostEdits( state );
		...
		const getAutosaveAttribute = ( 0, external_wp_data_namespaceObject.createRegistrySelector )(
			select => ( state, attributeName ) => {
				...
				const postType = getCurrentPostType( state );
				const postId = getCurrentPostId( state );
				const currentUserId = select( external_wp_coreData_namespaceObject.store ).getCurrentUser()
					?.id;
		...
		const isEditedPostEmpty = ( 0, external_wp_data_namespaceObject.createRegistrySelector )(
			select => state => {
				...
				if (
					blockName !== ( 0, external_wp_blocks_namespaceObject.getDefaultBlockName )() &&
					blockName !== ( 0, external_wp_blocks_namespaceObject.getFreeformContentHandlerName )()
				) {
					return false;
				}
				return ! getEditedPostContent( state );
		...
		external_wp_data_namespaceObject.createRegistrySelector )( select => state => {
			...
			return entitiesBeingSaved.some(
				entityRecord =>
					entityRecord.kind !== 'postType' || entityRecord.name !== type || entityRecord.key !== id
			);
		} );
			...
			select => state => {
				...
				if ( record ) {
					if ( typeof record.content === 'function' ) {
						...
						return ( 0, external_wp_blocks_namespaceObject.__unstableSerializeAndClean )(
							record.blocks
						);
		...
		function canUserUseUnfilteredHTML( state ) {
			return Boolean(
				getCurrentPost( state )._links?.hasOwnProperty( 'wp:action-unfiltered-html' )
			);
		}
		 ...
		 */
		const getEditorBlocks = rememo(
			state => {
				return (
					getEditedPostAttribute( state, 'blocks' ) ||
					( 0, external_wp_blocks_namespaceObject.parse )( getEditedPostContent( state ) )
				);
			},
		...
		function __experimentalGetTemplateInfo( state, template ) {
			...
			const templateIcon =
				__experimentalGetDefaultTemplatePartAreas( state ).find( item => area === item.area )
					?.icon || library_layout;
			return {
		...
		function getNotificationArgumentsForSaveFail( data ) {
			...
			let noticeMessage =
				! isPublished && publishStatus.indexOf( edits.status ) !== -1
					...
					: ( 0, external_wp_i18n_namespaceObject.__ )( 'Updating failed.' );
		...
		const setupEditor =
			...
			( { dispatch } ) => {
				...
				if ( isNewPost && template ) {
					...
					blocks = ( 0, external_wp_blocks_namespaceObject.synchronizeBlocksWithTemplate )(
						blocks,
						template
					);
				...
				if (
					edits &&
					Object.values( edits ).some( ( [ key, edit ] ) => {
						var _post$key$raw;
						return (
							edit !==
							( ( _post$key$raw = post[ key ]?.raw ) !== null && _post$key$raw !== void 0
								? _post$key$raw
								: post[ key ] )
						);
					} )
			...
			async ( { select, dispatch, registry } ) => {
				...
				const edits = {
					...
					...registry
						...
						.getEntityRecordNonTransientEdits( 'postType', previousRecord.type, previousRecord.id ),
					content,
				};
				dispatch( {
				...
				let error = registry
					.select( external_wp_coreData_namespaceObject.store )
					.getLastEntitySaveError( 'postType', previousRecord.type, previousRecord.id );
				if ( ! error ) {
			...
			( { select, dispatch, registry } ) => {
				...
				if ( __unstableShouldCreateUndoLevel !== false ) {
					...
					const noChange =
						registry
							.select( external_wp_coreData_namespaceObject.store )
							.getEditedEntityRecord( 'postType', type, id ).blocks === edits.blocks;
					if ( noChange ) {
						registry
							.dispatch( external_wp_coreData_namespaceObject.store )
		 ...
		 */
		const updateBlockListSettings = getBlockEditorAction( 'updateBlockListSettings' ); // CONCATENATED MODULE: ./node_modules/@wordpress/editor/build-module/store/index.js
					...
					( { attributes, setAttributes, ...props } ) => {
						...
						const mergedAttributes = ( 0, external_wp_element_namespaceObject.useMemo )(
							() => ( {
								...attributes,
								...Object.fromEntries(
									Object.entries( metaAttributes ).map( ( [ attributeKey, metaKey ] ) => [
										attributeKey,
										meta[ metaKey ],
									] )
								),
						...
						return ( 0, external_wp_element_namespaceObject.createElement )( BlockEdit, {
							...
							setAttributes: nextAttributes => {
								const nextMeta = Object.fromEntries(
									Object.entries(
										nextAttributes !== null && nextAttributes !== void 0 ? nextAttributes : {}
									)
										.filter(
		...
		function shimAttributeSource( settings ) {
			...
			const metaAttributes = Object.fromEntries(
				Object.entries(
					( _settings$attributes = settings.attributes ) !== null && _settings$attributes !== void 0
		...
		var external_wp_keyboardShortcuts_namespaceObject = window[ 'wp' ][ 'keyboardShortcuts' ]; // CONCATENATED MODULE: ./node_modules/@wordpress/editor/build-module/components/global-keyboard-shortcuts/index.js
		/**
		 * WordPress dependencies
		 */
		...
		function EditorKeyboardShortcuts() {
			const { redo, undo, savePost } = ( 0, external_wp_data_namespaceObject.useDispatch )(
				store_store
			);
			const { isEditedPostDirty, isPostSavingLocked } = ( 0,
		...
		external_wp_compose_namespaceObject.compose )( [
			( 0, external_wp_data_namespaceObject.withSelect )( ( select, ownProps ) => {
				...
				const { isEditedPostDirty, isEditedPostAutosaveable, isAutosavingPost, getEditorSettings } =
					select( store_store );
		...
		/* harmony default export */ var document_outline_item = TableOfContentsItem; // CONCATENATED MODULE: ./node_modules/@wordpress/editor/build-module/components/document-outline/index.js
		 ...
		 * Module constants
		 */
		const emptyHeadingContent = ( 0, external_wp_element_namespaceObject.createElement )(
			'em',
			null,
		...
		external_wp_compose_namespaceObject.compose )(
			( 0, external_wp_data_namespaceObject.withSelect )( select => {
				...
				return {
					...
					blocks: getBlocks(),
					isTitleSupported:
						( _postType$supports$ti = postType?.supports?.title ) !== null &&
		...
		/* harmony default export */ var check = ( 0, external_wp_data_namespaceObject.withSelect )(
			select => ( {
				blocks: select( external_wp_blockEditor_namespaceObject.store ).getBlocks(),
			} )
		)( DocumentOutlineCheck ); // CONCATENATED MODULE: external ["wp","keycodes"]
		...
		/* harmony default export */ var register_shortcuts = EditorKeyboardShortcutsRegister; // CONCATENATED MODULE: external ["wp","components"]
				...
				( 0, external_wp_element_namespaceObject.createElement )(
					external_wp_components_namespaceObject.CheckboxControl,
					{
						...
						label: ( 0, external_wp_element_namespaceObject.createElement )(
							'strong',
							null,
							( 0, external_wp_htmlEntities_namespaceObject.decodeEntities )( entityRecordTitle ) ||
		...
		const TRANSLATED_SITE_PROPERTIES = {
			...
			default_comment_status: ( 0, external_wp_i18n_namespaceObject.__ )(
				'Allow comments on new posts'
			),
		};
		const useIsDirty = () => {
			const { editedEntities, siteEdits } = ( 0, external_wp_data_namespaceObject.useSelect )(
				select => {
					const { __experimentalGetDirtyEntityRecords, getEntityRecordEdits } = select(
						external_wp_coreData_namespaceObject.store
					);
					return {
		...
		function EntitiesSavedStatesExtensible( {
			...
			const { createSuccessNotice, createErrorNotice, removeNotice } = ( 0,
			external_wp_data_namespaceObject.useDispatch )( external_wp_notices_namespaceObject.store );
			...
			const saveEnabled =
				saveEnabledProp !== null && saveEnabledProp !== void 0 ? saveEnabledProp : isDirty;
			const saveCheckedEntities = () => {
				const saveNoticeId = 'site-editor-save-success';
				removeNotice( saveNoticeId );
				const entitiesToSave = dirtyEntityRecords.filter( ( { kind, name, key, property } ) => {
					return ! unselectedEntities.some(
						elt =>
							elt.kind === kind && elt.name === name && elt.key === key && elt.property === property
					);
				} );
		...
		function useAutosaveNotice() {
			...
			external_wp_data_namespaceObject.useDispatch )( external_wp_notices_namespaceObject.store );
			const { editPost, resetEditorBlocks } = ( 0, external_wp_data_namespaceObject.useDispatch )(
				store_store
			);
			( 0, external_wp_element_namespaceObject.useEffect )( () => {
						...
						actions: [
							{
								...
								onClick() {
									const { content: editsContent, ...editsWithoutContent } = edits;
		...
		function useAutosavePurge() {
			...
			const wasEditedPostNew = ( 0, external_wp_compose_namespaceObject.usePrevious )(
				isEditedPostNew
			);
		...
		/* harmony default export */ var page_attributes_check = PageAttributesCheck; // CONCATENATED MODULE: ./node_modules/@wordpress/editor/build-module/components/post-type-support-check/index.js
		...
		function PostTypeSupportCheck( { children, supportKeys } ) {
			...
			if ( postType ) {
				isSupported = ( Array.isArray( supportKeys ) ? supportKeys : [ supportKeys ] ).some(
		...
		/* harmony default export */ var post_type_support_check = PostTypeSupportCheck; // CONCATENATED MODULE: ./node_modules/@wordpress/editor/build-module/components/page-attributes/order.js
		...
		function PageAttributesOrder() {
			const order = ( 0, external_wp_data_namespaceObject.useSelect )( select => {
				...
				return ( _select$getEditedPost =
					select( store_store ).getEditedPostAttribute( 'menu_order' ) ) !== null &&
					_select$getEditedPost !== void 0
					? _select$getEditedPost
					: 0;
			}, [] );
			const { editPost } = ( 0, external_wp_data_namespaceObject.useDispatch )( store_store );
			const [ orderInput, setOrderInput ] = ( 0, external_wp_element_namespaceObject.useState )(
				null
			);
		...
		var remove_accents_default = /*#__PURE__*/ __webpack_require__.n( remove_accents ); // CONCATENATED MODULE: ./node_modules/@wordpress/editor/build-module/utils/terms.js
		/**
		 * WordPress dependencies
		 */
		...
		function PageAttributesParent() {
			...
			const { isHierarchical, parentPostId, parentPostTitle, pageItems } = ( 0,
			external_wp_data_namespaceObject.useSelect )(
				select => {
					...
					const postIsHierarchical =
						( _pType$hierarchical = pType?.hierarchical ) !== null && _pType$hierarchical !== void 0
							? _pType$hierarchical
							: false;
					const query = {
						per_page: 100,
					...
					const mappedNodes = tree.map( treeNode => [
						{
							...
							label:
								'— '.repeat( level ) +
								( 0, external_wp_htmlEntities_namespaceObject.decodeEntities )( treeNode.name ),
		...
		/* harmony default export */ var page_attributes_parent = PageAttributesParent; // CONCATENATED MODULE: ./node_modules/@wordpress/editor/build-module/components/post-template/index.js
		...
		function PostTemplate() {
			...
			external_wp_data_namespaceObject.useSelect )( select => {
				...
				const { getEditedPostAttribute, getEditorSettings, getCurrentPostType } =
					select( store_store );
				const { getPostType } = select( external_wp_coreData_namespaceObject.store );
				return {
					...
					isViewable:
						( _getPostType$viewable = getPostType( getCurrentPostType() )?.viewable ) !== null &&
			...
			return ( 0, external_wp_element_namespaceObject.createElement )(
				external_wp_components_namespaceObject.SelectControl,
				{
					...
					options: Object.entries(
						availableTemplates !== null && availableTemplates !== void 0 ? availableTemplates : {}
					).map( ( [ templateSlug, templateName ] ) => ( {
						value: templateSlug,
						label: templateName,
					} ) ),
				}
		...
		function PostAuthorCombobox() {
			...
			const authorOptions = ( 0, external_wp_element_namespaceObject.useMemo )( () => {
				...
				const foundAuthor = fetchedAuthors.findIndex( ( { value } ) => postAuthor?.id === value );
		...
		function PostAuthorSelect() {
			const { editPost } = ( 0, external_wp_data_namespaceObject.useDispatch )( store_store );
			const { postAuthor, authors } = ( 0, external_wp_data_namespaceObject.useSelect )( select => {
				return {
		...
		function PostAuthor() {
			const showCombobox = ( 0, external_wp_data_namespaceObject.useSelect )( select => {
				const authors = select( external_wp_coreData_namespaceObject.store ).getUsers(
					AUTHORS_QUERY
				);
			...
			external_wp_data_namespaceObject.useSelect )( select => {
				...
				return {
					hasAssignAuthorAction:
						( _post$_links$wpActio = post._links?.[ 'wp:action-assign-author' ] ) !== null &&
		...
		function PostComments() {
			const commentStatus = ( 0, external_wp_data_namespaceObject.useSelect )( select => {
				var _select$getEditedPost;
				return ( _select$getEditedPost =
					select( store_store ).getEditedPostAttribute( 'comment_status' ) ) !== null &&
					_select$getEditedPost !== void 0
					? _select$getEditedPost
					: 'open';
			}, [] );
		...
		/* harmony default export */ var post_comments = PostComments; // CONCATENATED MODULE: ./node_modules/@wordpress/editor/build-module/components/post-excerpt/index.js
		...
		function PostExcerpt() {
			const excerpt = ( 0, external_wp_data_namespaceObject.useSelect )(
				select => select( store_store ).getEditedPostAttribute( 'excerpt' ),
				[]
			);
			const { editPost } = ( 0, external_wp_data_namespaceObject.useDispatch )( store_store );
			return ( 0, external_wp_element_namespaceObject.createElement )(
		...
		/* harmony default export */ var post_excerpt = PostExcerpt; // CONCATENATED MODULE: ./node_modules/@wordpress/editor/build-module/components/post-excerpt/check.js
		...
		function ThemeSupportCheck( { themeSupports, children, postType, supportKeys } ) {
			const isSupported = ( Array.isArray( supportKeys ) ? supportKeys : [ supportKeys ] ).some(
				key => {
					var _themeSupports$key;
					const supported =
						( _themeSupports$key = themeSupports?.[ key ] ) !== null &&
						_themeSupports$key !== void 0
							? _themeSupports$key
							: false;
		...
		external_wp_data_namespaceObject.withSelect )( select => {
			const { getThemeSupports } = select( external_wp_coreData_namespaceObject.store );
			const { getEditedPostAttribute } = select( store_store );
			return {
		...
		/* harmony default export */ var post_featured_image_check = PostFeaturedImageCheck; // CONCATENATED MODULE: ./node_modules/@wordpress/editor/build-module/components/post-featured-image/index.js
		...
		const DEFAULT_FEATURE_IMAGE_LABEL = ( 0, external_wp_i18n_namespaceObject.__ )(
			'Featured image'
		);
		const DEFAULT_SET_FEATURE_IMAGE_LABEL = ( 0, external_wp_i18n_namespaceObject.__ )(
			'Set featured image'
		);
		...
		function getMediaDetails( media, postId ) {
			...
			if (
				fallbackSize in
				( ( _media$media_details$2 = media?.media_details?.sizes ) !== null &&
		...
		function PostFeaturedImage( {
			...
			const [ isLoading, setIsLoading ] = ( 0, external_wp_element_namespaceObject.useState )(
				false
			);
			...
			const { mediaWidth, mediaHeight, mediaSourceUrl } = getMediaDetails( media, currentPostId );
								...
								render: ( { open } ) =>
									( 0, external_wp_element_namespaceObject.createElement )(
										...
										( 0, external_wp_element_namespaceObject.createElement )(
											...
											! featuredImageId &&
												! isLoading &&
												( postType?.labels?.set_featured_image || DEFAULT_SET_FEATURE_IMAGE_LABEL )
										),
										!! featuredImageId &&
											( 0, external_wp_element_namespaceObject.createElement )(
		...
		const applyWithSelect = ( 0, external_wp_data_namespaceObject.withSelect )( select => {
			const { getMedia, getPostType } = select( external_wp_coreData_namespaceObject.store );
			const { getCurrentPostId, getEditedPostAttribute } = select( store_store );
		...
		/* harmony default export */ var post_format_check = PostFormatCheck; // CONCATENATED MODULE: ./node_modules/@wordpress/editor/build-module/components/post-format/index.js
		...
		function PostLastRevisionCheck( { children } ) {
			const { lastRevisionId, revisionsCount } = ( 0, external_wp_data_namespaceObject.useSelect )(
				select => {
		...
		/* harmony default export */ var post_last_revision_check = PostLastRevisionCheck; // CONCATENATED MODULE: ./node_modules/@wordpress/editor/build-module/components/post-last-revision/index.js
		...
		/* harmony default export */ var post_last_revision = LastRevision; // CONCATENATED MODULE: ./node_modules/@wordpress/editor/build-module/components/post-locked-modal/index.js
		...
		function PostLockedModal() {
			...
			const { autosave, updatePostLock } = ( 0, external_wp_data_namespaceObject.useDispatch )(
				store_store
			);
			const {
				isLocked,
			...
			return ( 0, external_wp_element_namespaceObject.createElement )(
				...
				{
					title: isTakeover
						? ( 0, external_wp_i18n_namespaceObject.__ )( 'Someone else has taken over this post' )
						: ( 0, external_wp_i18n_namespaceObject.__ )( 'This post is already being edited' ),
					focusOnMount: true,
						...
						!! isTakeover &&
							( 0, external_wp_element_namespaceObject.createElement )(
								...
								( 0, external_wp_element_namespaceObject.createInterpolateElement )(
									...
									{
										strong: ( 0, external_wp_element_namespaceObject.createElement )(
											'strong',
											null
										),
										PreviewLink: ( 0, external_wp_element_namespaceObject.createElement )(
											external_wp_components_namespaceObject.ExternalLink,
											{
												href: previewLink,
											},
		...
		function PostPendingStatusCheck( { children } ) {
			const { hasPublishAction, isPublished } = ( 0, external_wp_data_namespaceObject.useSelect )(
				select => {
					var _getCurrentPost$_link;
					const { isCurrentPostPublished, getCurrentPost } = select( store_store );
					return {
						hasPublishAction:
							( _getCurrentPost$_link = getCurrentPost()._links?.[ 'wp:action-publish' ] ) !==
								null && _getCurrentPost$_link !== void 0
								? _getCurrentPost$_link
								: false,
		...
		/* harmony default export */ var post_pending_status_check = PostPendingStatusCheck; // CONCATENATED MODULE: ./node_modules/@wordpress/editor/build-module/components/post-pending-status/index.js
		...
		/* harmony default export */ var post_pending_status = PostPendingStatus; // CONCATENATED MODULE: ./node_modules/@wordpress/editor/build-module/components/post-pingbacks/index.js
		...
		function PostPingbacks() {
			const pingStatus = ( 0, external_wp_data_namespaceObject.useSelect )( select => {
				var _select$getEditedPost;
				return ( _select$getEditedPost =
					select( store_store ).getEditedPostAttribute( 'ping_status' ) ) !== null &&
					_select$getEditedPost !== void 0
					? _select$getEditedPost
					: 'open';
			}, [] );
		...
		/* harmony default export */ var post_pingbacks = PostPingbacks; // CONCATENATED MODULE: ./node_modules/@wordpress/editor/build-module/components/post-preview-button/index.js
	...
	`;
		...
		function PostPreviewButton( { className, textContent, forceIsAutosaveable, role, onPreview } ) {
			const { postId, currentPostLink, previewLink, isSaveable, isViewable } = ( 0,
			external_wp_data_namespaceObject.useSelect )( select => {
				var _postType$viewable;
		...
		/* harmony default export */ var label = ( 0, external_wp_compose_namespaceObject.compose )( [
			( 0, external_wp_data_namespaceObject.withSelect )( select => {
				var _getCurrentPost$_link;
				const {
				...
				return {
					...
					hasPublishAction:
						( _getCurrentPost$_link = getCurrentPost()._links?.[ 'wp:action-publish' ] ) !== null &&
						_getCurrentPost$_link !== void 0
							? _getCurrentPost$_link
							: false,
		...
		class PostPublishButton extends external_wp_element_namespaceObject.Component {
			...
			render() {
				...
				const isToggleDisabled =
					( isPublished || isSaving || ! isSaveable || ( ! isPublishable && ! forceIsDirty ) ) &&
					( ! hasNonPostEntityChanges || isSavingNonPostEntityChanges );
				let publishStatus;
				if ( ! hasPublishAction ) {
		...
		function PostVisibility( { onClose } ) {
			...
			const { status, visibility, password } = ( 0, external_wp_data_namespaceObject.useSelect )(
				select => ( {
			...
			const { editPost, savePost } = ( 0, external_wp_data_namespaceObject.useDispatch )(
				store_store
			);
			const [ hasPassword, setHasPassword ] = ( 0, external_wp_element_namespaceObject.useState )(
				!! password
			);
		...
		function usePostVisibilityLabel() {
			const visibility = ( 0, external_wp_data_namespaceObject.useSelect )( select =>
				select( store_store ).getEditedPostVisibility()
			);
		...
		function PostSchedule( { onClose } ) {
			...
			external_wp_element_namespaceObject.useState )( startOfMonth( new Date( postDate ) ) );
		...
		function getPostScheduleLabel( dateAttribute, { isFloating = false, now = new Date() } = {} ) {
			...
			if ( isSameDay( date, now ) ) {
				return ( 0, external_wp_i18n_namespaceObject.sprintf )(
					...
					( 0, external_wp_date_namespaceObject.dateI18n )(
						( 0, external_wp_i18n_namespaceObject._x )( 'g:i\xa0a', 'post schedule time format' ),
						date
					)
				);
			}
		...
		function FlatTermSelector( { slug } ) {
			...
			const debouncedSearch = ( 0, external_wp_compose_namespaceObject.useDebounce )(
				setSearch,
				500
			);
			const { terms, termIds, taxonomy, hasAssignAction, hasCreateAction, hasResolvedTerms } = ( 0,
			external_wp_data_namespaceObject.useSelect )(
				select => {
					...
					return {
						hasCreateAction: _taxonomy
							? ( _post$_links = post._links?.[ 'wp:action-create-' + _taxonomy.rest_base ] ) !==
									null && _post$_links !== void 0
								? _post$_links
								: false
							: false,
						hasAssignAction: _taxonomy
							? ( _post$_links2 = post._links?.[ 'wp:action-assign-' + _taxonomy.rest_base ] ) !==
									null && _post$_links2 !== void 0
								? _post$_links2
								: false
							: false,
						taxonomy: _taxonomy,
			...
			( 0, external_wp_element_namespaceObject.useEffect )( () => {
				if ( hasResolvedTerms ) {
					const newValues = ( terms !== null && terms !== void 0 ? terms : [] ).map( term =>
			...
			const suggestions = ( 0, external_wp_element_namespaceObject.useMemo )( () => {
				return ( searchResults !== null && searchResults !== void 0 ? searchResults : [] ).map(
					term => unescapeString( term.name )
				);
			}, [ searchResults ] );
			const { editPost } = ( 0, external_wp_data_namespaceObject.useDispatch )( store_store );
			const { saveEntityRecord } = ( 0, external_wp_data_namespaceObject.useDispatch )(
				external_wp_coreData_namespaceObject.store
			);
			...
			const newTermLabel =
				( _taxonomy$labels$add_ = taxonomy?.labels?.add_new_item ) !== null &&
				_taxonomy$labels$add_ !== void 0
					? _taxonomy$labels$add_
					: slug === 'post_tag'
					? ( 0, external_wp_i18n_namespaceObject.__ )( 'Add new tag' )
					: ( 0, external_wp_i18n_namespaceObject.__ )( 'Add new Term' );
			const singularName =
				( _taxonomy$labels$sing2 = taxonomy?.labels?.singular_name ) !== null &&
				_taxonomy$labels$sing2 !== void 0
					? _taxonomy$labels$sing2
					: slug === 'post_tag'
					? ( 0, external_wp_i18n_namespaceObject.__ )( 'Tag' )
					: ( 0, external_wp_i18n_namespaceObject.__ )( 'Term' );
		...
		external_wp_components_namespaceObject.withFilters )( 'editor.PostTaxonomyType' )(
			FlatTermSelector
		...
		const TagsPanel = () => {
			...
			return ( 0, external_wp_element_namespaceObject.createElement )(
				...
				( 0, external_wp_element_namespaceObject.createElement )( flat_term_selector, {
					slug: 'post_tag',
				} )
			);
		};
		...
		external_wp_compose_namespaceObject.compose )(
			( 0, external_wp_data_namespaceObject.withSelect )( select => {
				...
				const tagsTaxonomy = select( external_wp_coreData_namespaceObject.store ).getTaxonomy(
					'post_tag'
				);
				const tags =
					tagsTaxonomy && select( store_store ).getEditedPostAttribute( tagsTaxonomy.rest_base );
				return {
		...
		function PostFormatPanel() {
			const { currentPostFormat, suggestion } = ( 0, external_wp_data_namespaceObject.useSelect )(
				select => {
					var _select$getThemeSuppo;
					const { getEditedPostAttribute, getSuggestedPostFormat } = select( store_store );
					const supportedFormats =
						( _select$getThemeSuppo = select(
							external_wp_coreData_namespaceObject.store
						).getThemeSupports().formats ) !== null && _select$getThemeSuppo !== void 0
							? _select$getThemeSuppo
							: [];
					return {
		...
		function HierarchicalTermSelector( { slug } ) {
			...
			const [ showForm, setShowForm ] = ( 0, external_wp_element_namespaceObject.useState )(
				false
			);
			...
			const { hasCreateAction, hasAssignAction, terms, loading, availableTerms, taxonomy } = ( 0,
			external_wp_data_namespaceObject.useSelect )(
				select => {
					var _post$_links, _post$_links2;
					const { getCurrentPost, getEditedPostAttribute } = select( store_store );
			...
			const labelWithFallback = ( labelProperty, fallbackIsCategory, fallbackIsNotCategory ) => {
				...
				return ( _taxonomy$labels$labe = taxonomy?.labels?.[ labelProperty ] ) !== null &&
					_taxonomy$labels$labe !== void 0
					? _taxonomy$labels$labe
					: slug === 'category'
					? fallbackIsCategory
			...
			const filterLabel =
				( _taxonomy$labels$sear = taxonomy?.labels?.search_items ) !== null &&
				_taxonomy$labels$sear !== void 0
					? _taxonomy$labels$sear
					: ( 0, external_wp_i18n_namespaceObject.__ )( 'Search Terms' );
			const groupLabel =
				( _taxonomy$name = taxonomy?.name ) !== null && _taxonomy$name !== void 0
					? _taxonomy$name
		...
		function MaybeCategoryPanel() {
			const hasNoCategory = ( 0, external_wp_data_namespaceObject.useSelect )( select => {
				...
				const postTypeSupportsCategories =
					categoriesTaxonomy && categoriesTaxonomy.types.some( type => type === postType );
				const categories =
					categoriesTaxonomy &&
					select( store_store ).getEditedPostAttribute( categoriesTaxonomy.rest_base );
				...
				return (
					...
					( categories?.length === 0 ||
						( categories?.length === 1 && defaultCategory?.id === categories[ 0 ] ) )
				);
			}, [] );
			...
			return ( 0, external_wp_element_namespaceObject.createElement )(
				...
				( 0, external_wp_element_namespaceObject.createElement )( hierarchical_term_selector, {
					slug: 'category',
				} )
			);
		}
		/* harmony default export */ var maybe_category_panel = MaybeCategoryPanel; // CONCATENATED MODULE: ./node_modules/@wordpress/icons/build-module/library/upload.js

		/**
		 * WordPress dependencies
		 */

		const upload = ( 0, external_wp_element_namespaceObject.createElement )(
			external_wp_primitives_namespaceObject.SVG,
			{
				xmlns: 'http://www.w3.org/2000/svg',
				viewBox: '0 0 24 24',
		...
		/* harmony default export */ var library_upload = upload; // CONCATENATED MODULE: ./node_modules/@wordpress/editor/build-module/components/post-publish-panel/maybe-upload-media.js
		...
		function maybe_upload_media_PostFormatPanel() {
			const [ isUploading, setIsUploading ] = ( 0, external_wp_element_namespaceObject.useState )(
				false
			);
			const { editorBlocks, mediaUpload } = ( 0, external_wp_data_namespaceObject.useSelect )(
				select => ( {
					...
					mediaUpload: select( external_wp_blockEditor_namespaceObject.store ).getSettings()
						.mediaUpload,
				} ),
				[]
			);
			const externalImages = flattenBlocks( editorBlocks ).filter(
				block => block.name === 'core/image' && block.attributes.url && ! block.attributes.id
			);
			const { updateBlockAttributes } = ( 0, external_wp_data_namespaceObject.useDispatch )(
				external_wp_blockEditor_namespaceObject.store
			);
		...
		function PostPublishPanelPrepublish( { children } ) {
			const {
				...
				return {
					...
					siteHome:
						siteData.home &&
						( 0, external_wp_url_namespaceObject.filterURLForDisplay )( siteData.home ),
				};
			}, [] );
			...
			return ( 0, external_wp_element_namespaceObject.createElement )(
				...
				( 0, external_wp_element_namespaceObject.createElement )( 'p', null, prePublishBodyText ),
				( 0, external_wp_element_namespaceObject.createElement )(
					'div',
					{
				...
				hasPublishAction &&
					( 0, external_wp_element_namespaceObject.createElement )(
						...
						( 0, external_wp_element_namespaceObject.createElement )(
							...
							( 0, external_wp_element_namespaceObject.createElement )( PostVisibility, null )
						),
						( 0, external_wp_element_namespaceObject.createElement )(
							external_wp_components_namespaceObject.PanelBody,
							{
				...
				( 0, external_wp_element_namespaceObject.createElement )( PostFormatPanel, null ),
				( 0, external_wp_element_namespaceObject.createElement )( maybe_tags_panel, null ),
				( 0, external_wp_element_namespaceObject.createElement )( maybe_category_panel, null ),
				children
			);
		}
				...
				return ( 0, external_wp_element_namespaceObject.createElement )(
					...
					( 0, external_wp_element_namespaceObject.createElement )(
						...
						( 0, external_wp_element_namespaceObject.createElement )(
							...
							( 0, external_wp_htmlEntities_namespaceObject.decodeEntities )( post.title ) ||
		...
		class PostPublishPanel extends external_wp_element_namespaceObject.Component {
			...
			render() {
				...
				const { hasPublishAction, isDirty, isPostTypeViewable, ...propsForPanel } = additionalProps;
		...
		/* harmony default export */ var library_cloud = cloud; // CONCATENATED MODULE: ./node_modules/@wordpress/editor/build-module/components/post-saved-state/index.js
		 ...
		 */
		function PostSavedState( { forceIsDirty, showIconLabels = false } ) {
     ...
     */ showTooltip: true,
		...
		class PostSlug extends external_wp_element_namespaceObject.Component {
			constructor( { postSlug, postTitle, postID } ) {
				...
				this.state = {
					editedSlug:
						( 0, external_wp_url_namespaceObject.safeDecodeURIComponent )( postSlug ) ||
		...
		/* harmony default export */ var post_slug = ( 0, external_wp_compose_namespaceObject.compose )(
			[
				( 0, external_wp_data_namespaceObject.withSelect )( select => {
					const { getCurrentPost, getEditedPostAttribute } = select( store_store );
		...
		external_wp_compose_namespaceObject.compose )( [
			( 0, external_wp_data_namespaceObject.withSelect )( select => {
				...
				return {
					hasStickyAction:
						( _post$_links$wpActio = post._links?.[ 'wp:action-sticky' ] ) !== null &&
						_post$_links$wpActio !== void 0
							? _post$_links$wpActio
							: false,
					postType: select( store_store ).getCurrentPostType(),
		...
		] )( PostSticky ); // CONCATENATED MODULE: ./node_modules/@wordpress/editor/build-module/components/post-switch-to-draft-button/index.js
		...
		] )( PostSwitchToDraftButton ); // CONCATENATED MODULE: external ["wp","privateApis"]

		var external_wp_privateApis_namespaceObject = window[ 'wp' ][ 'privateApis' ]; // CONCATENATED MODULE: ./node_modules/@wordpress/editor/build-module/lock-unlock.js
		/**
		 * WordPress dependencies
		 */
		...
		const { ReusableBlocksRenameHint } = unlock(
			external_wp_blockEditor_namespaceObject.privateApis
		);
		function PostSyncStatus() {
			const { syncStatus, postType } = ( 0, external_wp_data_namespaceObject.useSelect )(
				select => {
					const { getEditedPostAttribute } = select( store_store );
			...
			return ( 0, external_wp_element_namespaceObject.createElement )(
				...
				( 0, external_wp_element_namespaceObject.createElement )(
					'span',
					null,
					( 0, external_wp_i18n_namespaceObject.__ )( 'Sync status' )
				),
				( 0, external_wp_element_namespaceObject.createElement )(
					'div',
					null,
					syncStatus === 'unsynced'
						? ( 0, external_wp_i18n_namespaceObject.__ )( 'Not synced' )
						: ( 0, external_wp_i18n_namespaceObject.__ )( 'Fully synced' )
				)
			);
		}
		function PostSyncStatusModal() {
			...
			const [ syncType, setSyncType ] = ( 0, external_wp_element_namespaceObject.useState )(
				undefined
			);
			const { postType, isNewPost } = ( 0, external_wp_data_namespaceObject.useSelect )( select => {
				const { getEditedPostAttribute, isCleanNewPost } = select( store_store );
				return {
					postType: getEditedPostAttribute( 'type' ),
		...
		function PostTaxonomies( {
			...
			return visibleTaxonomies.map( taxonomy => {
				...
				return ( 0, external_wp_element_namespaceObject.createElement )(
					...
					taxonomyWrapper(
						( 0, external_wp_element_namespaceObject.createElement )( TaxonomyComponent, {
							slug: taxonomy.slug,
						} ),
						taxonomy
					)
		...
		function PostTextEditor() {
			...
			const { content, blocks, type, id } = ( 0, external_wp_data_namespaceObject.useSelect )(
				select => {
			...
			const { editEntityRecord } = ( 0, external_wp_data_namespaceObject.useDispatch )(
				external_wp_coreData_namespaceObject.store
			);
			...
			return ( 0, external_wp_element_namespaceObject.createElement )(
				...
				( 0, external_wp_element_namespaceObject.createElement )( lib /* default */.Z, {
					autoComplete: 'off',
					dir: 'auto',
					value: value,
					onChange: event => {
		...
		function PostTitle( _, forwardedRef ) {
			...
			const [ isSelected, setIsSelected ] = ( 0, external_wp_element_namespaceObject.useState )(
				false
			);
			const { editPost } = ( 0, external_wp_data_namespaceObject.useDispatch )( store_store );
			...
			external_wp_data_namespaceObject.useSelect )( select => {
				const { getEditedPostAttribute, isCleanNewPost: _isCleanNewPost } = select( store_store );
				const { getSettings } = select( external_wp_blockEditor_namespaceObject.store );
				const { titlePlaceholder, hasFixedToolbar: _hasFixedToolbar } = getSettings();
				return {
			...
			const decodedPlaceholder =
				( 0, external_wp_htmlEntities_namespaceObject.decodeEntities )( placeholder ) ||
		...
		function PostTrash() {
			const { isNew, isDeleting, postId } = ( 0, external_wp_data_namespaceObject.useSelect )(
				select => {
		...
		function PostURL( { onClose } ) {
			const { isEditable, postSlug, viewPostLabel, postLink, permalinkPrefix, permalinkSuffix } =
				( 0, external_wp_data_namespaceObject.useSelect )( select => {
					var _post$_links$wpActio;
					const post = select( store_store ).getCurrentPost();
					...
					const hasPublishAction =
						( _post$_links$wpActio = post?._links?.[ 'wp:action-publish' ] ) !== null &&
						_post$_links$wpActio !== void 0
							? _post$_links$wpActio
							: false;
					return {
					...
					( 0, external_wp_element_namespaceObject.createElement )(
						external_wp_components_namespaceObject.TextControl,
						{
							...
							help: ( 0, external_wp_element_namespaceObject.createElement )(
								external_wp_element_namespaceObject.Fragment,
								null,
								( 0, external_wp_i18n_namespaceObject.__ )( 'The last part of the URL.' ),
								' ',
								( 0, external_wp_element_namespaceObject.createElement )(
									external_wp_components_namespaceObject.ExternalLink,
									{
		...
		function TimeToRead() {
			...
			const minutesToReadString =
				minutesToRead === 0
					? ( 0, external_wp_element_namespaceObject.createInterpolateElement )(
							( 0, external_wp_i18n_namespaceObject.__ )( '<span>< 1</span> minute' ),
							{
				...
				( 0, external_wp_element_namespaceObject.createElement )(
					...
					( 0, external_wp_element_namespaceObject.createElement )(
						...
						( 0, external_wp_element_namespaceObject.createElement )(
							...
							( 0, external_wp_element_namespaceObject.createElement )(
								...
								( 0, external_wp_element_namespaceObject.createElement )( WordCount, null )
							),
							( 0, external_wp_element_namespaceObject.createElement )(
								'li',
								{
								...
								( 0, external_wp_element_namespaceObject.createElement )(
									...
									( 0, external_wp_element_namespaceObject.createElement )( CharacterCount, null )
								)
							),
							( 0, external_wp_element_namespaceObject.createElement )(
								'li',
								...
								( 0, external_wp_element_namespaceObject.createElement )( TimeToRead, null )
							),
							( 0, external_wp_element_namespaceObject.createElement )(
								'li',
								{
					...
					headingCount > 0 &&
						( 0, external_wp_element_namespaceObject.createElement )(
							external_wp_element_namespaceObject.Fragment,
							null,
							( 0, external_wp_element_namespaceObject.createElement )( 'hr', null ),
							( 0, external_wp_element_namespaceObject.createElement )(
								'h2',
								{
							...
							( 0, external_wp_element_namespaceObject.createElement )( document_outline, {
		...
		function UnsavedChangesWarning() {
			...
			external_wp_data_namespaceObject.useSelect )( external_wp_coreData_namespaceObject.store );
			( 0, external_wp_element_namespaceObject.useEffect )( () => {
				/**
				 * Warns the user if there are unsaved changes before leaving the editor.
				 *
		...
		var external_wp_patterns_namespaceObject = window[ 'wp' ][ 'patterns' ]; // CONCATENATED MODULE: ./node_modules/@wordpress/editor/build-module/components/provider/with-registry-provider.js
		/**
		 * WordPress dependencies
		 */
		...
		/* harmony default export */ var with_registry_provider = withRegistryProvider; // CONCATENATED MODULE: ./node_modules/@wordpress/editor/build-module/components/media-categories/index.js
		...
		const coreMediaFetch = async ( query = {} ) => {
			const mediaItems = await ( 0, external_wp_data_namespaceObject.resolveSelect )(
				external_wp_coreData_namespaceObject.store
			).getMediaItems( {
				...query,
		...
		function useBlockEditorSettings( settings, hasTemplate ) {
			...
			const {
				...
				var _canUser;
				const { canUserUseUnfilteredHTML, getCurrentPostType } = select( store_store );
				const isWeb = external_wp_element_namespaceObject.Platform.OS === 'web';
				const { canUser, getEntityRecord, getUserPatternCategories } = select(
					external_wp_coreData_namespaceObject.store
				);
				...
				return {
					...
					hasUploadPermissions:
						( _canUser = canUser( 'create', 'media' ) ) !== null && _canUser !== void 0
							? _canUser
							: true,
			...
			const settingsBlockPatterns =
				( _settings$__experimen = settings.__experimentalAdditionalBlockPatterns ) !== null &&
				_settings$__experimen !== void 0
					? _settings$__experimen
					: // WP 6.0
					  settings.__experimentalBlockPatterns; // WP 5.9
			const settingsBlockPatternCategories =
				( _settings$__experimen2 = settings.__experimentalAdditionalBlockPatternCategories ) !==
					null && _settings$__experimen2 !== void 0
			...
			const blockPatterns = ( 0, external_wp_element_namespaceObject.useMemo )(
				() =>
					[ ...( settingsBlockPatterns || [] ), ...( restBlockPatterns || [] ) ]
						...
						.filter( ( { postTypes } ) => {
							return (
								! postTypes || ( Array.isArray( postTypes ) && postTypes.includes( postType ) )
							);
						} ),
			...
			return ( 0, external_wp_element_namespaceObject.useMemo )(
				() => ( {
					...
					__experimentalFetchLinkSuggestions: ( search, searchOptions ) =>
						( 0, external_wp_coreData_namespaceObject.__experimentalFetchLinkSuggestions )(
							search,
							searchOptions,
							settings
						),
		...
		/* harmony default export */ var use_block_editor_settings = useBlockEditorSettings; // CONCATENATED MODULE: ./node_modules/@wordpress/editor/build-module/components/provider/index.js
		...
		const ExperimentalEditorProvider = with_registry_provider(
			( {
				...
				external_wp_data_namespaceObject.useSelect )( select => {
					const { getEditorSettings, getEditorSelection, __unstableIsEditorReady } =
						select( store_store );
					return {
				...
				external_wp_coreData_namespaceObject.useEntityBlockEditor )( 'postType', type, {
					id,
				} );
				...
				const { updatePostLock, setupEditor, updateEditorSettings, __experimentalTearDownEditor } =
					( 0, external_wp_data_namespaceObject.useDispatch )( store_store );
				const { createWarningNotice } = ( 0, external_wp_data_namespaceObject.useDispatch )(
					external_wp_notices_namespaceObject.store
				);
					...
					( 0, external_wp_element_namespaceObject.createElement )(
						...
						( 0, external_wp_element_namespaceObject.createElement )(
							...
							( 0, external_wp_element_namespaceObject.createElement )(
								...
								children,
								( 0, external_wp_element_namespaceObject.createElement )( PatternsMenuItems, null )
							)
						)
					)
				);
		...
		const TextEditorGlobalKeyboardShortcuts = EditorKeyboardShortcuts; // CONCATENATED MODULE: ./node_modules/@wordpress/editor/build-module/utils/url.js
