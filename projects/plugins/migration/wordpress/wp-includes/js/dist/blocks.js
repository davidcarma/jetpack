    ...
    */
	...
	! ( function () {
		...
		__webpack_require__.d( private_selectors_namespaceObject, {
			getBootstrappedBlockType: function () {
				return getBootstrappedBlockType;
			},
			getSupportedStyles: function () {
				return getSupportedStyles;
			},
			getUnprocessedBlockTypes: function () {
				return getUnprocessedBlockTypes;
			},
		} );
		...
		__webpack_require__.d( actions_namespaceObject, {
			__experimentalReapplyBlockFilters: function () {
				return __experimentalReapplyBlockFilters;
			},
			...
			reapplyBlockTypeFilters: function () {
		...
		__webpack_require__.d( private_actions_namespaceObject, {
			addBootstrappedBlockType: function () {
				return addBootstrappedBlockType;
			},
			addUnprocessedBlockType: function () {
				return addUnprocessedBlockType;
			},
		} ); // CONCATENATED MODULE: external ["wp","data"]

		var external_wp_data_namespaceObject = window[ 'wp' ][ 'data' ]; // CONCATENATED MODULE: ./node_modules/tslib/tslib.es6.mjs
		/******************************************************************************
...
***************************************************************************** */
		...
		var a11y_o = function ( o ) {
			...
			a11y_t = function ( t ) {
				return 0.2126 * a11y_o( t.r ) + 0.7152 * a11y_o( t.g ) + 0.0722 * a11y_o( t.b );
			};
		...
		var external_wp_privateApis_namespaceObject = window[ 'wp' ][ 'privateApis' ]; // CONCATENATED MODULE: ./node_modules/@wordpress/blocks/build-module/lock-unlock.js
		/**
		...
		function setFreeformContentHandlerName( blockName ) {
			( 0, external_wp_data_namespaceObject.dispatch )( store ).setFreeformFallbackBlockName(
				blockName
			);
		}
		...
		function setUnregisteredTypeHandlerName( blockName ) {
			( 0, external_wp_data_namespaceObject.dispatch )( store ).setUnregisteredFallbackBlockName(
				blockName
			);
		}
		...
		const getChildBlockNames = blockName => {
			return ( 0, external_wp_data_namespaceObject.select )( store ).getChildBlockNames(
				blockName
			);
		};
		...
		const getBlockVariations = ( blockName, scope ) => {
			return ( 0, external_wp_data_namespaceObject.select )( store ).getBlockVariations(
				blockName,
				scope
			);
		};
		...
		const unregisterBlockVariation = ( blockName, variationName ) => {
			( 0, external_wp_data_namespaceObject.dispatch )( store ).removeBlockVariations(
				blockName,
				variationName
			);
		...
		const getBlockFromExample = ( name, example ) => {
			...
			return createBlock(
				name,
				example.attributes,
				( ( _example$innerBlocks = example.innerBlocks ) !== null && _example$innerBlocks !== void 0
			...
			if ( 'background' in icon ) {
				...
				return {
					...
					foreground: icon.foreground
						? icon.foreground
						: ICON_COLORS.find( iconColor => getColorContrast( iconColor ) === maxContrast ),
		...
		function __experimentalGetBlockAttributesNamesByRole( name, role ) {
			...
			return attributesNames.filter(
				attributeName => attributes[ attributeName ]?.__experimentalRole === role
			);
		}
		...
		function blockStyles( state = {}, action ) {
			...
			switch ( action.type ) {
								...
								( [ name, blockType ] ) => {
									...
									return [
										...
										getUniqueItemsByName( [
											...
											...( ( _state$blockType$name = state[ blockType.name ] ) !== null &&
				...
				case 'ADD_BLOCK_STYLES':
					return {
						...
						[ action.blockName ]: getUniqueItemsByName( [
							...( ( _state$action$blockNa = state[ action.blockName ] ) !== null &&
				...
				case 'REMOVE_BLOCK_STYLES':
					return {
						...state,
						[ action.blockName ]: ( ( _state$action$blockNa2 = state[ action.blockName ] ) !==
							null && _state$action$blockNa2 !== void 0
			...
			switch ( action.type ) {
							...
							Object.entries( keyBlockTypesByName( action.blockTypes ) ).map(
								( [ name, blockType ] ) => {
									...
									return [
										name,
										getUniqueItemsByName( [
											...( ( _blockType$variations = blockType.variations ) !== null &&
											...
											...( ( _state$blockType$name2 = state[ blockType.name ] ) !== null &&
				...
				case 'ADD_BLOCK_VARIATIONS':
					return {
						...state,
						[ action.blockName ]: getUniqueItemsByName( [
							...( ( _state$action$blockNa3 = state[ action.blockName ] ) !== null &&
				...
				case 'REMOVE_BLOCK_VARIATIONS':
					return {
						...state,
						[ action.blockName ]: ( ( _state$action$blockNa4 = state[ action.blockName ] ) !==
							null && _state$action$blockNa4 !== void 0
		...
		function categories( state = DEFAULT_CATEGORIES, action ) {
			switch ( action.type ) {
				...
				case 'UPDATE_CATEGORY': {
					...
					const categoryToChange = state.find( ( { slug } ) => slug === action.slug );
					if ( categoryToChange ) {
		...
		var remove_accents_default = /*#__PURE__*/ __webpack_require__.n( remove_accents ); // CONCATENATED MODULE: external ["wp","compose"]
		var external_wp_compose_namespaceObject = window[ 'wp' ][ 'compose' ]; // CONCATENATED MODULE: ./node_modules/@wordpress/blocks/build-module/store/utils.js
		/**
		...
		const getNormalizedBlockType = ( state, nameOrType ) =>
			'string' === typeof nameOrType ? selectors_getBlockType( state, nameOrType ) : nameOrType;
		...
		function isMatchingSearchTerm( state, nameOrType, searchTerm ) {
			...
			return (
				...
				( typeof blockType.description === 'string' && isSearchMatch( blockType.description ) )
			);
		}
		 ...
		 */
		const getSupportedStyles = rememo(
			( state, name, element ) => {
				if ( ! name ) {
				...
				Object.keys( __EXPERIMENTAL_STYLE_PROPERTY ).forEach( styleName => {
					...
					if ( __EXPERIMENTAL_STYLE_PROPERTY[ styleName ].requiresOptOut ) {
						if (
							__EXPERIMENTAL_STYLE_PROPERTY[ styleName ].support[ 0 ] in blockType.supports &&
		...
		unlock( store ).registerPrivateActions( private_actions_namespaceObject ); // CONCATENATED MODULE: external ["wp","blockSerializationDefaultParser"]
		...
		function getSaveElement( blockTypeOrName, attributes, innerBlocks = [] ) {
			...
			if (
				...
				( 0, external_wp_hooks_namespaceObject.hasFilter )( 'blocks.getSaveContent.extraProps' ) &&
		...
		function fixCustomClassname( blockAttributes, blockType, innerHTML ) {
			if ( hasBlockSupport( blockType, 'customClassName', true ) ) {
				...
				const { className: omittedClassName, ...attributesSansClassName } = blockAttributes;
		...
		function applyBlockDeprecatedVersions( block, rawBlock, blockType ) {
			...
			for ( let i = 0; i < deprecatedDefinitions.length; i++ ) {
				...
				if ( migrate ) {
					...
					[ migratedAttributes = parsedAttributes, migratedInnerBlocks = block.innerBlocks ] =
						migrated;
				}
				block = {
					...block,
		...
		function parser_parse( content, options ) {
			return ( 0, external_wp_blockSerializationDefaultParser_namespaceObject.parse )(
				content
		...
		function moreCommentConverter( node, doc ) {
			...
			if (
				...
				].forEach( element => element && wrapperNode.insertBefore( element, node.parentNode ) );
		...
		function canHaveAnchor( node, schema ) {
			...
			return (
				tag in
				( ( _schema$figure$childr2 = schema?.figure?.children?.a?.children ) !== null &&
		...
		var cjs_default = /*#__PURE__*/ __webpack_require__.n( cjs ); // CONCATENATED MODULE: ./node_modules/@wordpress/blocks/build-module/api/raw-handling/utils.js
		/**
		 * External dependencies
		 */
		...
		function getBlockContentSchemaFromTransforms( transforms, context ) {
			const phrasingContentSchema = ( 0, external_wp_dom_namespaceObject.getPhrasingContentSchema )(
				context
			);
			const schemaArgs = {
		...
		function phrasingContentReducer( node, doc ) {
			...
			if ( node.nodeName === 'SPAN' && node.style ) {
				const { fontWeight, fontStyle, textDecorationLine, textDecoration, verticalAlign } =
					node.style;
