/******/ ( function () {
	...
	function applyFormat( value, format, startIndex = value.start, endIndex = value.end ) {
		...
		if ( startIndex === endIndex ) {
			const startFormat = newFormats[ startIndex ]?.find( ( { type } ) => type === format.type );
	 ...
	 * @return {RichTextValue} A rich text value.
	 */
	function createFromElement( { element, range, isEditableTree, preserveWhiteSpace } ) {
	...
	function removeFormat( value, formatType, startIndex = value.start, endIndex = value.end ) {
		...
		// format.
		if ( startIndex === endIndex ) {
			const format = newFormats[ startIndex ]?.find( ( { type } ) => type === formatType );
			if ( format ) {
	 ...
	 */
	function apply( { value, current, prepareEditableTree, __unstableDomOnly, placeholder } ) {
	...
	function to_html_string_getLastChild( { children } ) {
	...
	function to_html_string_getParent( { parent } ) {
		return parent;
	}
	function to_html_string_isText( { text } ) {
		return typeof text === 'string';
	}
	function to_html_string_getText( { text } ) {
		return text;
	}
	...
	function createElementHTML( { type, attributes, object, children } ) {
		...
		for ( const key in attributes ) {
			...
			external_wp_escapeHtml_namespaceObject.escapeAttribute )( attributes[ key ] ) }"`;
		}
		if ( object ) {
		...
		return children
			.map( child => {
				...
				return child.text === undefined
					...
					: ( 0, external_wp_escapeHtml_namespaceObject.escapeEditableHTML )( child.text );
			} )
			.join( '' );
	...
	function useInputAndSelection( props ) {
		...
		return ( 0, external_wp_compose_namespaceObject.useRefEffect )( element => {
			...
			function onInput( event ) {
				...
				const { record, applyRecord, createRecord, handleChange } = propsRef.current;
			...
			function handleSelectionChange() {
				const { record, applyRecord, createRecord, onSelectionChange } = propsRef.current;
	...
	function FormatEdit( { formatTypes, onChange, onFocus, value, forwardedRef } ) {
