/******/ ( function () {
	...
	function getFormatBoundary( value, format, startIndex = value.start, endIndex = value.end ) {
		...
		const formatAtStart = newFormats[ startIndex ]?.find( ( { type } ) => type === format.type );
		const formatAtEnd = newFormats[ endIndex ]?.find( ( { type } ) => type === format.type );
	...
	const LINK_SETTINGS = [
		...external_wp_blockEditor_namespaceObject.__experimentalLinkControl.DEFAULT_LINK_SETTINGS,
		{
			id: 'nofollow',
	...
	function InlineLinkUI( {
		...
		function onChangeLink( nextValue ) {
			...
			const didToggleSetting =
				linkValue.opensInNewTab !== nextValue.opensInNewTab && nextValue.url === undefined;
			...
			const linkFormat = createLinkFormat( {
				...
				type: nextValue.type,
				id:
					nextValue.id !== undefined && nextValue.id !== null ? String( nextValue.id ) : undefined,
	...
	/* harmony default export */ var inline = InlineLinkUI; // CONCATENATED MODULE: ./node_modules/@wordpress/format-library/build-module/link/index.js
	...
	function link_Edit( { isActive, activeAttributes, value, onChange, onFocus, contentRef } ) {
		const [ addingLink, setAddingLink ] = ( 0, external_wp_element_namespaceObject.useState )(
			false
		);
		function addLink() {
	...
	const formatStrikethrough = ( 0, external_wp_element_namespaceObject.createElement )(
		external_wp_primitives_namespaceObject.SVG,
		{
			xmlns: 'http://www.w3.org/2000/svg',
			viewBox: '0 0 24 24',
	...
	function getActiveColors( value, name, colorSettings ) {
		const activeColorFormat = ( 0, external_wp_richText_namespaceObject.getActiveFormat )(
			value,
			name
		);
	...
	function setColors( value, name, colorSettings, colors ) {
		...
		if ( color ) {
			const colorObject = ( 0, external_wp_blockEditor_namespaceObject.getColorObjectByColorValue )(
				colorSettings,
				color
			);
			if ( colorObject ) {
   ...
   */
		const cachedRect = ( 0, external_wp_blockEditor_namespaceObject.useCachedTruthy )(
			popoverAnchor.getBoundingClientRect()
		);
	...
	function TextColorEdit( { value, onChange, isActive, activeAttributes, contentRef } ) {
		const allowCustomControl = ( 0, external_wp_blockEditor_namespaceObject.useSetting )(
			'color.custom'
		);
		const colors =
			( 0, external_wp_blockEditor_namespaceObject.useSetting )( 'color.palette' ) || EMPTY_ARRAY;
		const [ isAddingColor, setIsAddingColor ] = ( 0, external_wp_element_namespaceObject.useState )(
			false
		);
		const enableIsAddingColor = ( 0, external_wp_element_namespaceObject.useCallback )(
			() => setIsAddingColor( true ),
			[ setIsAddingColor ]
		);
		const disableIsAddingColor = ( 0, external_wp_element_namespaceObject.useCallback )(
			() => setIsAddingColor( false ),
			[ setIsAddingColor ]
		);
		const colorIndicatorStyle = ( 0, external_wp_element_namespaceObject.useMemo )(
			() =>
				fillComputedColors( contentRef.current, getActiveColors( value, text_color_name, colors ) ),
			[ value, colors ]
		);
			...
			( 0, external_wp_element_namespaceObject.createElement )(
				external_wp_blockEditor_namespaceObject.RichTextToolbarButton,
				{
					...
					onClick: hasColorsToChoose
						...
						: () =>
								onChange(
									( 0, external_wp_richText_namespaceObject.removeFormat )( value, text_color_name )
								),
					role: 'menuitemcheckbox',
				}
			),
	...
	const language = ( 0, external_wp_element_namespaceObject.createElement )(
		external_wp_primitives_namespaceObject.SVG,
		{
			xmlns: 'http://www.w3.org/2000/svg',
			viewBox: '0 0 24 24',
	...
	/* harmony default export */ var library_language = language; // CONCATENATED MODULE: ./node_modules/@wordpress/format-library/build-module/language/index.js
	...
	function language_Edit( { isActive, value, onChange, contentRef } ) {
		...
		return ( 0, external_wp_element_namespaceObject.createElement )(
			...
			( 0, external_wp_element_namespaceObject.createElement )(
				external_wp_blockEditor_namespaceObject.RichTextToolbarButton,
				{
					...
					onClick: () => {
						if ( isActive ) {
							onChange(
								( 0, external_wp_richText_namespaceObject.removeFormat )( value, language_name )
							);
						} else {
							togglePopover();
						}
			...
			isPopoverVisible &&
				( 0, external_wp_element_namespaceObject.createElement )( InlineLanguageUI, {
					value: value,
					onChange: onChange,
