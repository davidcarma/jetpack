...
class X_Usage_Controller extends Base_Controller {
	 ...
	 * Get X usage data.
	 *
	 * Returns the quota limit, plan type, and per-month usage breakdown.
	 * Each month with data includes counts of 'used' (done) and 'pending'
	 * (scheduled/future) shares. Only months with entries are included.
	 *
	 * For paid plans, each month has its own quota. For free plans, the
	 * limit applies across all months (lifetime).
	 *
	 * @param WP_REST_Request $request Full details about the request.
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'usage'   => array(
					'type'                 => 'object',
					'description'          => __( 'Per-month usage breakdown keyed by yyyy-mm. Only months with data are included.', 'jetpack-publicize-pkg' ),
